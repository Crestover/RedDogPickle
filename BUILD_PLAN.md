...
- [x] 7c. **Padel module (Manual scoring)** — `sports/padel.ts` + `padelValidators.ts` implement padel's real win condition: first to 6 games, straight win-by-2, **no tiebreak and no cap** (sets can extend to 9-7, 10-8, ...; corrected after initial build assumed a tennis-style 6-6→7-6 tiebreak, which is wrong for how this group plays). Single-set-per-recording; each set rated independently. `RecordGameForm.tsx` rewired to route validation through `sportConfig` (`getSportConfig`) instead of hardcoded pickleball imports — was a real gap, not just a padel placeholder. A migration (`m18.0`) *was* needed despite the original "client-side only" plan: `sessions.target_points_default`/`games.target_points` had `CHECK` constraints hard-blocking any value outside pickleball's 11/15/21 — not just a loose backstop as assumed. Sport badge added to group dashboard (light-touch theming). Verified end-to-end against dev DB (group creation → session → 6-2 accepted, 11-7 rejected, 9-7 accepted+recorded, ratings + standings correct). **Deferred:** Courts Mode padel support, DB-level enforcement of the full win condition (DB still just checks `winner >= target_points`).
- [x] 7d. **Admin screen** — `/rd-admin` (non-obvious path) + `ADMIN_PASSWORD`-gated signed HttpOnly cookie (`src/lib/admin/auth.ts`, HMAC-SHA256, 4h session, no session table). Writes go through a service-role Supabase client (`src/lib/supabase/adminServer.ts`) used only after the password check passes — needed since anon RLS has no UPDATE policy on `players.hidden`. Group list (join_code, sport, player count, session count, last session date — aggregated in-memory, no new RPC), create-group form (sport assignment, auto-slugified join code), per-group player hide/unhide toggle. Verified end-to-end against dev DB. **New required env vars for prod:** `ADMIN_PASSWORD`, `ADMIN_SESSION_SECRET`, `SUPABASE_SERVICE_ROLE_KEY` (see `.env.example`) — must be set in Vercel before this ships to production, with real values, not the dev-test ones.
- [x] 7e. **Per-player game history page** — `/g/[join_code]/players/[player_id]`, reachable by tapping a name in `LeaderboardCard` (main leaderboard + both session standings screens). Cross-session query (`game_players` → `games` join), stat summary reusing a new shared `getStatLabels()` helper (extracted from `LeaderboardCard`, also used for sport-aware Sets/Games labels), game cards adapted from `EndedSessionGames` with a per-player W/L pill + session date link. `LeaderboardCard` gained a `playerBasePath` prop (plain string, not a function — Client Components can't receive function props from Server Components). `/v/` view-only pages don't get this link; no read-only mirror was in scope. Verified end-to-end for both sports against dev DB.
