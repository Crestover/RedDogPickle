...
/**
 * PlayerGameHistoryList — cross-session game log for a single player.
 *
 * Adapted from EndedSessionGames' scoreboard styling, but each card is
 * relative to the given player: a W/L pill (not a per-session G# badge,
 * since sequence_num resets per session and isn't meaningful across them)
 * plus the session name/date so games from different sessions are
 * distinguishable. Voided games hidden by default, same as elsewhere.
 */
...
export default function PlayerGameHistoryList({ playerId, joinCode, games }: Props) {
  ...
  if (games.length === 0) {
    return <p className="text-sm text-gray-400 text-center py-6">No games recorded yet.</p>;
  }
    ...
    <div>
      ...
      <div className="space-y-2.5">
        {displayGames.map((game) => {
          ...
          const gamePlayers = Array.isArray(game.game_players) ? game.game_players : [];
          const myTeam = gamePlayers.find((gp) => gp.player_id === playerId)?.team === "B" ? "B" : "A";
          ...
          const iWon = (myTeam === "A" && aWins) || (myTeam === "B" && bWins);
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                {/* W/L pill — left anchor, relative to this player */}
                <div className="flex flex-col items-center gap-1 shrink-0">
                  <span
                    ...
                    style={{ minWidth: "44px", minHeight: "44px", backgroundColor: !isVoided && iWon ? "#167659" : undefined }}
                  >
                    {iWon ? "W" : "L"}
                  </span>
                  {isVoided && (
                ...
                {/* Content — names + score + session/date */}
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline justify-between">
                    ...
                    <span className="text-lg font-extrabold tabular-nums tracking-tight shrink-0 ml-2">
                      ...
                      <span className="text-gray-400">{padScore(loserScore)}</span>
                    </span>
                  </div>
                  ...
                  <div className="mt-1">
                    <Link
                      href={`/g/${joinCode}/session/${game.session_id}/games`}
                      className="text-[11px] font-mono text-gray-400 hover:text-gray-600 transition-colors truncate"
                    >
                      {game.session_name} &middot; {dateLabel}
                    </Link>
                  </div>
                </div>
              </div>
