-- ============================================================
-- M17.0: Hidden Players
--
-- Adds a `hidden` flag to the players table. Hidden players
-- participate fully in sessions, games, and RDR calculations
-- but are excluded from all leaderboard and standings display
-- surfaces. Visibility is group-scoped (one flag per player
-- record, which belongs to exactly one group).
--
-- Changes:
--   1. Add hidden column to players (default false)
--   2. Update get_group_stats to exclude hidden players
--   3. Update get_session_stats to exclude hidden players
--
-- Design note:
--   Filtering is applied at the final result stage (WHERE on
--   the players JOIN), not in the stats aggregation subquery.
--   This preserves full data integrity: visible players' stats
--   continue to include all games vs hidden players.
-- ============================================================


-- ── 1. Add hidden column to players ─────────────────────────

ALTER TABLE public.players
  ADD COLUMN IF NOT EXISTS hidden boolean NOT NULL DEFAULT false;


-- ── 2. Update get_group_stats (exclude hidden players) ───────
-- Return type unchanged — CREATE OR REPLACE is safe.
...
BEGIN
  ...
  SELECT
    ...
    pr.rating_deviation,   -- currently unused by UI; included for future confidence-based sorting/filtering
    pr.last_played_at
  FROM (
    SELECT
      v.player_id,
...
-- ── 3. Update get_session_stats (exclude hidden players) ─────
-- Return type unchanged — CREATE OR REPLACE is safe.
