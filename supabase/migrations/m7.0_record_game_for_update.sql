-- ============================================================
-- M7.0: Add FOR UPDATE lock to record_game
--
-- Fixes:
--   C-1: Race condition on sequence_num (concurrent calls can
--        both read the same MAX(sequence_num) under READ COMMITTED)
--   M-2: TOCTOU between session liveness check and game insert
--        (end_session can slip between the SELECT and INSERT)
--
-- Solution: Lock the session row with FOR UPDATE before any reads.
-- This serializes concurrent record_game calls on the same session.
-- ============================================================

-- Clean up existing signature to prevent overload conflicts
...
BEGIN
  -- *** M7.0: Lock the session row to serialize concurrent game inserts ***
  -- This prevents both the sequence_num race (C-1) and the TOCTOU gap (M-2)
  -- where end_session could commit between our liveness check and INSERT.
  ...
  -- Safe from race conditions due to FOR UPDATE lock above
